...
void QPropertyBindingPrivatePtr::reset(QtPrivate::RefCounted* ptr) noexcept
{
    if (ptr != d)
    {
        ...
        if (old && (--old->ref == 0)) QPropertyBindingPrivate::destroyAndFreeMemory(static_cast<QPropertyBindingPrivate*>(d));
    }
}
...
void QPropertyBindingDataPointer::addObserver(QPropertyObserver* observer)
{
    if (auto* binding = bindingPtr())
    {
        ...
        if (observer->next) observer->next->prev = &observer->next;
...
{
    if (firstObserver) firstObserver.unlink();
    if (vtable->size) vtable->destroy(reinterpret_cast<std::byte*>(this) + sizeof(QPropertyBindingPrivate));
}

void QPropertyBindingPrivate::unlinkAndDeref()
{
    propertyDataPtr = nullptr;
    if (--ref == 0) destroyAndFreeMemory(this);
}
...
void QPropertyBindingPrivate::markDirtyAndNotifyObservers()
{
    ...
    auto guard = qScopeGuard([&]() { eagerlyUpdating = false; });
    bool knownIfChanged = false;
    if (requiresEagerEvaluation())
    {
        // these are compat properties that we will need to evaluate eagerly
        if (!evaluateIfDirtyAndReturnTrueIfValueChanged(propertyDataPtr)) return;
        knownIfChanged = true;
    }
    if (firstObserver) firstObserver.notify(this, propertyDataPtr, knownIfChanged);
    if (hasStaticObserver) staticObserverCallback(propertyDataPtr);
}

bool QPropertyBindingPrivate::evaluateIfDirtyAndReturnTrueIfValueChanged_helper(const QUntypedPropertyData* data,
                                                                                QBindingStatus* status)
{
    ...
    {
        changed = staticBindingWrapper(
            int(metaTypeId), mutable_data,
            {vtable, reinterpret_cast<std::byte*>(this) + QPropertyBindingPrivate::getSizeEnsuringAlignment()});
    }
    else
    {
        changed = vtable->call(metaTypeId, mutable_data,
                               reinterpret_cast<std::byte*>(this) + QPropertyBindingPrivate::getSizeEnsuringAlignment());
    }
...
QUntypedPropertyBinding::QUntypedPropertyBinding(int metaTypeId, const BindingFunctionVTable* vtable, void* function,
                                                 const QPropertyBindingSourceLocation& location)
{
    std::byte* mem = new std::byte[QPropertyBindingPrivate::getSizeEnsuringAlignment() + vtable->size]();
    d = new (mem) QPropertyBindingPrivate(metaTypeId, vtable, std::move(location));
...
QUntypedPropertyBinding::QUntypedPropertyBinding(QUntypedPropertyBinding&& other) : d(std::move(other.d)) {}

QUntypedPropertyBinding::QUntypedPropertyBinding(const QUntypedPropertyBinding& other) : d(other.d) {}
...
QUntypedPropertyBinding::QUntypedPropertyBinding(QPropertyBindingPrivate* priv) : d(priv) {}
...
{
    if (!d) return QPropertyBindingError();
    return static_cast<QPropertyBindingPrivate*>(d.get())->bindingError();
}

int QUntypedPropertyBinding::valueMetaTypeId() const
{
    if (!d) return 0;
    return static_cast<QPropertyBindingPrivate*>(d.get())->valueMetaTypeId();
}
...
QPropertyBindingData::~QPropertyBindingData()
{
    ...
    if (auto binding = d.bindingPtr()) binding->unlinkAndDeref();
}
                                                         ...
                                                         QtPrivate::QPropertyBindingWrapper guardCallback)
{
    ...
    if (newBinding)
    {
        ...
        if (observer) newBindingRaw->prependObserver(observer);
        ...
        if (newBindingRaw->requiresEagerEvaluation())
        {
            ...
            if (changed) observer.notify(newBindingRaw, propertyDataPtr, /*alreadyKnownToHaveChanged=*/true);
        }
    }
    else if (observer)
    {
    ...
    if (oldBinding) static_cast<QPropertyBindingPrivate*>(oldBinding.data())->detachFromProperty();
...
BindingEvaluationState::BindingEvaluationState(QPropertyBindingPrivate* binding, QBindingStatus* status) : binding(binding)
{
    QBindingStatus* s = status;
    if (!s) s = &bindingStatus;
...
CurrentCompatProperty::CurrentCompatProperty(QBindingStatus* status, QUntypedPropertyData* property) : property(property)
{
    // store a pointer to the currentBindingEvaluationState to avoid a TLS lookup in
    // the destructor (as these come with a non zero cost)
...
void QPropertyBindingData::removeBinding_helper()
{
    ...
    d_ptr = 0;
    if (observer) d.setObservers(observer.ptr);
...
{
    QPropertyBindingDataPointer d{this};
    if (QPropertyObserverPointer observer = d.firstObserver()) observer.notify(d.bindingPtr(), propertyDataPtr);
}
...
{
    if (ptr->next.tag() == QPropertyObserver::ObserverNotifiesAlias) ptr->aliasedPropertyData = nullptr;
    if (ptr->next) ptr->next->prev = ptr->prev;
    if (ptr->prev) ptr->prev.setPointer(ptr->next.data());
    ptr->next = nullptr;
    ptr->prev.clear();
}
...
struct [[nodiscard]] QPropertyObserverNodeProtector
{
    ...
    QPropertyObserverNodeProtector(QPropertyObserver * observer)
    {
        ...
        if (next) next->prev = &m_placeHolder.next;
...
void QPropertyObserverPointer::notify(QPropertyBindingPrivate* triggeringBinding, QUntypedPropertyData* propertyDataPtr,
                                      bool alreadyKnownToHaveChanged)
{
...
QPropertyBindingError::QPropertyBindingError(const QPropertyBindingError& other) : d(other.d) {}
...
QPropertyBindingError::QPropertyBindingError(QPropertyBindingError&& other) : d(std::move(other.d)) {}
...
{
    if (!d) return QPropertyBindingError::NoError;
    return d->type;
}
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> QObjectBindableProperty<Class, T, offset,
  Callback>::QObjectBindableProperty()

  Constructs a property with a default constructed instance of T.
*/

/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> explicit QObjectBindableProperty<Class, T, offset,
  Callback>::QObjectBindableProperty(const T &initialValue)
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> explicit QObjectBindableProperty<Class, T, offset,
  Callback>::QObjectBindableProperty(T &&initialValue)
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> QObjectBindableProperty<Class, T, offset,
  Callback>::QObjectBindableProperty(Class *owner, const QPropertyBinding<T> &binding)
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> QObjectBindableProperty<Class, T, offset,
  Callback>::QObjectBindableProperty(Class *owner, QPropertyBinding<T> &&binding)
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> template <typename Functor> QObjectBindableProperty<Class,
  T, offset, Callback>::QObjectBindableProperty(Functor &&f)
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> T QObjectBindableProperty<Class, T, offset,
  Callback>::value() const
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> void QObjectBindableProperty<Class, T, offset,
  Callback>::setValue(parameter_type newValue) \fn template <typename Class, typename T, auto offset, auto Callback> void
  QObjectBindableProperty<Class, T, offset, Callback>::setValue(rvalue_ref newValue)
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> QPropertyBinding<T> QObjectBindableProperty<Class, T,
  offset, Callback>::setBinding(const QPropertyBinding<T> &newBinding)
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> template <typename Functor> QPropertyBinding<T>
  QObjectBindableProperty<Class, T, offset, Callback>::setBinding(Functor f) \overload
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> QPropertyBinding<T> bool QObjectBindableProperty<Class, T,
  offset, Callback>::setBinding(const QUntypedPropertyBinding &newBinding) \overload
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> bool QObjectBindableProperty<Class, T, offset,
  Callback>::hasBinding() const
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> template <typename Functor> QPropertyChangeHandler<T,
  Functor> QObjectBindableProperty<Class, T, offset, Callback>::onValueChanged(Functor f)
...
/*!
  \fn template <typename Class, typename T, auto offset, auto Callback> template <typename Functor> QPropertyChangeHandler<T,
  Functor> QObjectBindableProperty<Class, T, offset, Callback>::subscribe(Functor f)
...
/*!
  \fn template <typename T> template <typename Functor> QPropertyChangeHandler<T, Functor>
  QPropertyAlias<T>::onValueChanged(Functor f)
...
struct QBindingStoragePrivate
{
    ...
    QPropertyBindingData* get(const QUntypedPropertyData* data)
    {
        ...
        while (p[index].data)
        {
            if (p[index].data == data) return &p[index].bindingData;
            ++index;
