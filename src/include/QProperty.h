...
struct QPROPERTY_BINDING_EXPORT QPropertyBindingSourceLocation
{
...
class QPROPERTY_BINDING_EXPORT QPropertyBindingError
{
   public:
    enum Type
    {
    ...
    bool hasError() const { return d.data() != nullptr; }
    Type type() const;
...
class QPROPERTY_BINDING_EXPORT QUntypedPropertyBinding
{
   public:
    ...
    QUntypedPropertyBinding(int metaTypeId, const BindingFunctionVTable* vtable, void* function,
                            const QPropertyBindingSourceLocation& location);

    template <typename Functor>
    QUntypedPropertyBinding(int metaTypeId, Functor&& f, const QPropertyBindingSourceLocation& location)
        : QUntypedPropertyBinding(metaTypeId, &QtPrivate::bindingFunctionVTable<std::remove_reference_t<Functor>>, &f, location)
    {
    }
...
class QPropertyBinding : public QUntypedPropertyBinding
{
   public:
    ...
    QPropertyBinding(Functor&& f, const QPropertyBindingSourceLocation& location)
        : QUntypedPropertyBinding(int(qMetaTypeId<PropertyType>()),
                                  &QtPrivate::bindingFunctionVTable<std::remove_reference_t<Functor>, PropertyType>, &f, location)
    {
    }

    template <typename Property, typename = typename Property::InheritsQUntypedPropertyData>
    QPropertyBinding(const Property& property) : QUntypedPropertyBinding(property.bindingData().binding())
    {
    }

    // Internal
    explicit QPropertyBinding(const QUntypedPropertyBinding& binding) : QUntypedPropertyBinding(binding) {}
};
...
class QPROPERTY_BINDING_EXPORT QPropertyObserver : public QPropertyObserverBase
{
   public:
   ...
   protected:
    ...
    QUntypedPropertyData* aliasedProperty() const { return aliasedPropertyData; }
...
class QProperty : public QPropertyData<T>
{
   ...
   public:
    ...
    explicit QProperty(const QPropertyBinding<T>& binding) : QProperty() { setBinding(binding); }
#ifndef Q_CLANG_QDOC
    template <typename Functor>
    ...
    {
        if (!newBinding.isNull() && newBinding.valueMetaTypeId() != qMetaTypeId<T>()) return false;
    ...
    QPropertyBinding<T> binding() const { return QPropertyBinding<T>(*this); }

    QPropertyBinding<T> takeBinding() { return QPropertyBinding<T>(d.setBinding(QUntypedPropertyBinding(), this)); }
   ...
   private:
    void notify() { d.notifyObservers(this); }
...
struct QBindableInterface
{
    ...
    using MakeBinding
        = QUntypedPropertyBinding (*)(const QUntypedPropertyData* d, const QPropertyBindingSourceLocation& location);
    using SetObserver = void (*)(const QUntypedPropertyData* d, QPropertyObserver* observer);
    using GetMetaType = int (*)();
    Getter getter;
    Setter setter;
    ...
    GetMetaType metaTypeId;
};

template <typename Property, typename R>
struct MSVC_V141_Bug
{
    R operator()() { return static_cast<const Property*>(m_d)->value(); }
    const QUntypedPropertyData* m_d{nullptr};
};
    ...
    static constexpr QBindableInterface iface
        = {[](const QUntypedPropertyData* d, void* value) -> void {
           ...
           [](const QUntypedPropertyData* d, const QPropertyBindingSourceLocation& location) -> QUntypedPropertyBinding {
               return Qt::makePropertyBinding(MSVC_V141_Bug<Property, T>{d}, location);
           },
   ...
   public:
    ...
    static constexpr QBindableInterface iface
        = {[](const QUntypedPropertyData* d, void* value) -> void {
           ...
           [](const QUntypedPropertyData* d) -> QUntypedPropertyBinding { return static_cast<const Property*>(d)->binding(); },
...
class QUntypedBindable
{
   protected:
    ...
    constexpr QUntypedBindable(QUntypedPropertyData* d, const QtPrivate::QBindableInterface* i) : data(d), iface(i) {}

   public:
    constexpr QUntypedBindable() = default;
    template <typename Property>
    QUntypedBindable(Property* p)
        : data(const_cast<std::remove_cv_t<Property>*>(p)), iface(&QtPrivate::QBindableInterfaceForProperty<Property>::iface)
    {
        Q_ASSERT(data && iface);
    }
    ...
    {
        if (iface) iface->setObserver(data, observer);
    }
    ...
    {
        if (!iface->getBinding) return QUntypedPropertyBinding();
        return iface->getBinding(data);
    }
    bool setBinding(const QUntypedPropertyBinding& binding)
    {
        if (!iface->setBinding) return false;
        if (!binding.isNull() && binding.valueMetaTypeId() != iface->metaTypeId()) return false;
        iface->setBinding(data, binding);
        return true;
    }
    bool hasBinding() const { return !binding().isNull(); }
};
...
class QBindable : public QUntypedBindable
{
    ...
    constexpr QBindable(QUntypedPropertyData* d, const QtPrivate::QBindableInterface* i) : QUntypedBindable(d, i) {}

   public:
    using QUntypedBindable::QUntypedBindable;
    explicit QBindable(const QUntypedBindable& b) : QUntypedBindable(b)
    {
        if (iface && iface->metaTypeId() != qMetaTypeId<T>())
        {
            data = nullptr;
            iface = nullptr;
        }
    ...
    QPropertyBinding<T> binding() const { return static_cast<QPropertyBinding<T>&&>(QUntypedBindable::binding()); }
    using QUntypedBindable::setBinding;
    QPropertyBinding<T> setBinding(const QPropertyBinding<T>& binding)
    {
        Q_ASSERT(!iface || binding.isNull() || binding.valueMetaTypeId() == iface->metaTypeId());
    ...
    {
        if (iface) iface->setter(data, &value);
    }
};
...
{
    Q_DISABLE_COPY(QPropertyAlias)
    const QtPrivate::QBindableInterface* iface = nullptr;

   public:
    QPropertyAlias(QProperty<T>* property)
        : QPropertyObserver(property), iface(&QtPrivate::QBindableInterfaceForProperty<QProperty<T>>::iface)
    {
        if (iface) iface->setObserver(aliasedProperty(), this);
    }

    template <typename Property, typename = typename Property::InheritsQUntypedPropertyData>
    QPropertyAlias(Property* property)
        : QPropertyObserver(property), iface(&QtPrivate::QBindableInterfaceForProperty<Property>::iface)
    {
        if (iface) iface->setObserver(aliasedProperty(), this);
    }

    QPropertyAlias(QPropertyAlias<T>* alias) : QPropertyObserver(alias->aliasedProperty()), iface(alias->iface)
    {
        if (iface) iface->setObserver(aliasedProperty(), this);
    }

    QPropertyAlias(const QBindable<T>& property) : QPropertyObserver(property.data), iface(property.iface)
    {
        if (iface) iface->setObserver(aliasedProperty(), this);
    }
    ...
    {
        T t = T();
        if (auto* p = aliasedProperty()) iface->getter(p, &t);
        return t;
    }
    ...
    {
        if (auto* p = aliasedProperty()) iface->setter(p, &newValue);
    }
    ...
    bool hasBinding() const { return QBindable<T>(aliasedProperty(), iface).hasBinding(); }

    QPropertyBinding<T> binding() const { return QBindable<T>(aliasedProperty(), iface).binding(); }

    QPropertyBinding<T> takeBinding() { return QBindable<T>(aliasedProperty(), iface).takeBinding(); }
    ...
    bool isValid() const { return aliasedProperty() != nullptr; }
};
