...
class QPropertyBindingPrivatePtr
{
   public:
    ...
    {
        if (d && (--d->ref == 0)) destroyAndFreeMemory();
    }
    void destroyAndFreeMemory();

    explicit QPropertyBindingPrivatePtr(T* data) noexcept : d(data)
    {
        if (d) d->addRef();
    }
    QPropertyBindingPrivatePtr(const QPropertyBindingPrivatePtr& o) noexcept : d(o.d)
    {
        if (d) d->addRef();
    }
    ...
    void swap(QPropertyBindingPrivatePtr& other) noexcept { qSwap(d, other.d); }
    ...
    friend bool operator==(const QPropertyBindingPrivatePtr& p1, const T* ptr) noexcept { return p1.d == ptr; }
    friend bool operator!=(const QPropertyBindingPrivatePtr& p1, const T* ptr) noexcept { return p1.d != ptr; }
    friend bool operator==(const T* ptr, const QPropertyBindingPrivatePtr& p2) noexcept { return ptr == p2.d; }
    friend bool operator!=(const T* ptr, const QPropertyBindingPrivatePtr& p2) noexcept { return ptr != p2.d; }
    friend bool operator==(const QPropertyBindingPrivatePtr& p1, std::nullptr_t) noexcept { return !p1; }
    friend bool operator!=(const QPropertyBindingPrivatePtr& p1, std::nullptr_t) noexcept { return p1; }
    friend bool operator==(std::nullptr_t, const QPropertyBindingPrivatePtr& p2) noexcept { return !p2; }
    friend bool operator!=(std::nullptr_t, const QPropertyBindingPrivatePtr& p2) noexcept { return p2; }
...
{
    using CallFn = bool (*)(int, QUntypedPropertyData*, void*);
    ...
    static constexpr std::enable_if_t<!std::is_invocable_v<Callable>, BindingFunctionVTable> createFor()
    {
        static_assert(alignof(Callable) <= alignof(std::max_align_t), "Bindings do not support overaligned functors!");
        return {/*call=*/[](int metaTypeId, QUntypedPropertyData* dataPtr, void* f) {
                    if constexpr (!std::is_invocable_v<Callable>)
                    {
                        // we got an untyped callable
                        static_assert(std::is_invocable_r_v<bool, Callable, int, QUntypedPropertyData*>);
                        auto untypedEvaluationFunction = static_cast<Callable*>(f);
                        return std::invoke(*untypedEvaluationFunction, metaTypeId, dataPtr);
                    }
                },
                /*destroy*/ [](void* f) { static_cast<Callable*>(f)->~Callable(); },
                /*moveConstruct*/ [](void* addr, void* other) { new (addr) Callable(std::move(*static_cast<Callable*>(other))); },
    ...
    static constexpr std::enable_if_t<std::is_invocable_v<Callable>, BindingFunctionVTable> createFor()
    {
        ...
        return {/*call=*/[](int metaTypeId, QUntypedPropertyData* dataPtr, void* f) {
                    ...
                    {
                        if (newValue == propertyPtr->valueBypassingBindings()) return false;
                    }
...
using QPropertyBindingWrapper = bool (*)(int, QUntypedPropertyData* dataPtr, QPropertyBindingFunction);

class QPROPERTY_BINDING_EXPORT QPropertyBindingData
{
   ...
   public:
    ...
    QUntypedPropertyBinding setBinding(const QUntypedPropertyBinding& newBinding, QUntypedPropertyData* propertyDataPtr,
                                       QPropertyObserverCallback staticObserverCallback = nullptr,
                                       QPropertyBindingWrapper bindingWrapper = nullptr);

    QPropertyBindingPrivate* binding() const
    {
        if (d_ptr & BindingBit) return reinterpret_cast<QPropertyBindingPrivate*>(d_ptr - BindingBit);
        return nullptr;
    }
    ...
    void removeBinding()
    {
        if (hasBinding()) removeBinding_helper();
    }
...
class QTagPreservingPointerToPointer
{
   public:
    ...
    QTagPreservingPointerToPointer(T** ptr) : d(reinterpret_cast<quintptr*>(ptr)) {}
    ...
    T* get() const { return reinterpret_cast<T*>(*d & QTaggedPointer<T, Tag>::pointerMask()); }
